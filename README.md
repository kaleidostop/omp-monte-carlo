# Вычисление объёма фигуры с помощью параллельного метода Монте-Карло

### Инструментарий
- Язык С++17
- стандарт OpenMP 2.0. 
- IDE Clion
- компилятор cygwin64 3.5.0 С++
- процессор Intel Core i3-8145U CPU @ 2.10GHz (2 ядра, 4 логических процессора)

### Постановка задачи
Реализовать и оптимизировать программу, вычисляющую объем фигуры методом Монте-Карло с использованием многопоточности. 

Фигура задается в файле hit.cpp (в данном эксперименте – piriform с параметром a = 2)
...
```sh
.\omp_montecarlo.exe --omp-threads 4 --input in.txt --output out.txt
```

#### Вывод программы
В стандартный поток вывода пишется время выполнения программы (в миллисекундах), в выходной файл записывается объём фигуры
...
Для ускорения этот код далее был модифицирован, чтобы на каждой итерации менялись не 3 координаты, а только одна.
...
### Наилучший результат работы программы
при N = 100000000:
- V = 1.25654
- Time(4 thread(s)): 2711.93 ms 

### Итог
Удалось ускорить расчёт объёма методом Монте-Карло за счёт параллелизма OpenMP, выбора быстрого PCG-генератора и статического распределения итераций с размером чанка 1. Производительность хорошо масштабируется: на процессоре с большим количеством ядер и частотой результат был бы ещё быстрее.
